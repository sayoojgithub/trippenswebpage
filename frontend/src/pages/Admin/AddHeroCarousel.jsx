...
export default function AddHeroCarousel() {
  // ---- form state ----
  const [categoryOption, setCategoryOption] = useState(null);
  const [categories, setCategories] = useState([]); // [{value,label},...]
  const [file, setFile] = useState(null);
  const [preview, setPreview] = useState("");
  const [existingImage, setExistingImage] = useState(""); // when editing
  ...
  // fetch categories for select (pick a generous limit or expose a dedicated endpoint)
  ...
  const openFilePicker = () => {
    ...
    fileInputRef.current.value = ""; // allow re-select same file
  ...
  const handleSubmit = async (e) => {
    ...
    if (!categoryOption?.value && !isEditing) {
    ...
    try {
      ...
      if (isEditing && editId) {
        // update image only
        await API.patch(`/admin/hero-carousel/${editId}/image`, { imageUrl });
        setSuccessMsg("Hero slide updated successfully.");
      } else {
        // create
        await API.post("/admin/hero-carousel", {
  ...
  // ---- fetch hero slides (backend pagination, search by category name) ----
  const fetchSlides = async () => {
    setLoading(true);
    try {
      const { data } = await API.get("/admin/hero-carousel", {
  ...
  const startEdit = (row) => {
    // lock category (can’t change), allow image update only
    setIsEditing(true);
    setEditId(row._id);
    setExistingImage(row.imageUrl || "");
    setPreview("");
    setFile(null);
    // reflect current category for UX (but disabled)
    setCategoryOption({
      value: row.category?._id || row.category,
      label: row.category?.name || row.categoryName || "",
    });
    if (fileInputRef.current) fileInputRef.current.value = "";
  ...
  const toggleStatus = async (id, nextVal) => {
    try {
      ...
      await API.patch(`/admin/hero-carousel/${id}/status`, {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <Select
            ...
            options={categories}
            isDisabled={isEditing} // lock on edit
            ...
            styles={{
              ...
              option: (base, state) => ({
                ...base,
                backgroundColor: state.isFocused ? "rgb(31,41,55)" : "transparent",
                color: "#e5e7eb",
                cursor: "pointer",
              }),
            }}
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            ...
            {submitting ? (isEditing ? "Updating…" : "Saving…") : (isEditing ? "Update Slide" : "Create Slide")}
          </button>
        </div>
      </form>

      {/* --------- TABLE (search + pagination + edit image only) --------- */}
      <div className="mt-10">
                    ...
                    <td className="px-4 py-3">
                      <label className="inline-flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={(e) => toggleStatus(row._id, e.target.checked)}
                        />
