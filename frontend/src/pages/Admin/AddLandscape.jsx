...
export default function AddLandscape() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) {
      setErrorMsg("Landscape name is required.");
      return;
    }
    ...
    try {
      ...
      if (isEditing && editId) {
        // update
        await API.patch(`/admin/landscapes/${editId}`, {
          // name is locked while editing (per requirement), but you can enable it by including name here
          description: description.trim(),
          coverImage: coverImageUrl,
        });
        setSuccessMsg("Landscape updated successfully.");
      } else {
        // create
        await API.post("/admin/landscapes", {
          name: name.trim(),
          description: description.trim(),
          coverImage: coverImageUrl,
          // activeStatus defaults to true on backend
        });
        setSuccessMsg("Landscape created successfully.");
      }
  ...
  const fetchLandscapes = async () => {
    setLoading(true);
    try {
      const { data } = await API.get("/admin/landscapes", {
      ...
      console.error("Fetch landscapes failed:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleStatus = async (id, nextVal) => {
    try {
      ...
      await API.patch(`/admin/landscapes/${id}/status`, {
  ...
  const startEdit = async (row) => {
    try {
      ...
      const land = row; // using current row data is enough here
      setIsEditing(true);
      setEditId(land._id);
      setName(land.name);
      setDescription(land.description || "");
      setExistingCover(land.coverImage || "");
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold">
            {isEditing ? "Edit Landscape" : "Add Landscape"}
          </h2>
          {isEditing && (
            <button
              type="button"
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Short description for this landscape"
              ...
              {loading ? (
                ...
                rows.map((land) => (
                  <tr key={land._id}>
                    ...
                    <td className="px-4 py-3 font-medium">{land.name}</td>
                    ...
                    <td className="px-4 py-3">
                      <label className="inline-flex items-center cursor-pointer">
                        <input
                          ...
                          checked={!!land.activeStatus}
                          onChange={(e) =>
                            toggleStatus(land._id, e.target.checked)
                          }
                        />
