...
export default function AddAward() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) {
      setErrorMsg("Award name is required.");
      return;
    }
    ...
    try {
      ...
      if (isEditing && editId) {
        await API.patch(`/admin/awards/${editId}`, payload);
        setSuccessMsg("Award updated successfully.");
      } else {
        await API.post("/admin/awards", payload);
        setSuccessMsg("Award created successfully.");
      }

      resetForm();
      setPage(1); // ensure we see the newest item
      setRefreshKey((k) => k + 1);
    } catch (err) {
      const msg =
  ...
  // ---- fetch awards (backend pagination) ----
  const fetchAwards = async () => {
    ...
    try {
      ...
      console.error("Fetch awards failed:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleStatus = async (id, nextVal) => {
    try {
      ...
      await API.patch(`/admin/awards/${id}/status`, { activeStatus: nextVal });
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g., Travelers' Choice 2025"
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  >
                    ...
                    {preview ? "Change image" : existingImage ? "Replace image" : "Add image"}
                  </button>
                  {(preview || existingImage) && (
                    <button
                      type="button"
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            ...
            {submitting ? (isEditing ? "Updating…" : "Saving…") : isEditing ? "Update Award" : "Create Award"}
          </button>
        </div>
      </form>
                    ...
                    <td className="px-4 py-3">
                      <label className="inline-flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={(e) => toggleStatus(a._id, e.target.checked)}
                        />
