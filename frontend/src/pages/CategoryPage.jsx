...
// reveal that re-runs when deps change
...
export default function CategoryPage() {
  ...
  const rootRef = useReveal("[data-reveal]", [category?._id, tours.length]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError("");
        const { data } = await PUBLICAPI.get(`/public/categories/${categoryId}`, {
        ...
        console.error(e);
        if (mounted) setError("Could not load this category.");
      } finally {
    ...
    <section ref={rootRef} className="relative" style={{ fontFamily: THEME.font, background: THEME.bg }}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 pt-12 sm:pt-16 md:pt-20 text-center">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold leading-tight"
            style={{ color: THEME.brown }}>
          All <span style={{ color: THEME.gold }}>Tours</span>
        </h1>
      </div>
      ...
      <div className="mt-6 sm:mt-10 md:mt-12" data-reveal>
        <EnquiryForm />
      </div> */}
